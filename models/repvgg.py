...
class LWRNetF(nn.Module):
    def __init__(self, n_classes=6):
        super(LWRNetF, self).__init__()
        self.in_planes = 64
        self.stage0 = RepVGGBlock(in_channels=3, out_channels=self.in_planes, kernel_size=3, stride=1, padding=1,
                                  deploy=False)
        self.stage1 = self._make_stage(64, 1, stride=2)
        self.stage2 = self._make_stage(128, 1, stride=2)
        self.stage3 = self._make_stage(256, 2, stride=2)
        self.stage4 = self._make_stage(512, 1, stride=2)
        self.convtrans1 = nn.ConvTranspose2d(512, 256, 2, stride=2)
        self.in_planes = 256
        self.conv1 = self._make_stage(256, 1, stride=1)
        self.convtrans2 = nn.ConvTranspose2d(256, 128, 2, stride=2)
        self.in_planes = 128
        self.conv2 = self._make_stage(128, 1, stride=1)
        self.convtrans3 = nn.ConvTranspose2d(128, 64, 2, stride=2)
        self.in_planes = 64
        self.conv3 = self._make_stage(64, 1, stride=1)
        self.convtrans4 = nn.ConvTranspose2d(64, 32, 2, stride=2)
        self.in_planes = 32
        self.conv4 = self._make_stage(32, 1, stride=1)
        self.conv = nn.Conv2d(32, n_classes, 1, stride=1, padding=0)
    ...
    def _make_stage(self, planes, num_blocks, stride):
        ...
        for stride in strides:
            blocks.append(RepVGGBlock(in_channels=self.in_planes, out_channels=planes, kernel_size=3,
                                      stride=stride, padding=1, deploy=False))
...
if __name__ == '__main__':
    model = LWRNetF()
    data = torch.randn(2, 3, 1024, 1024)
